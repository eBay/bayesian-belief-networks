...
class UndirectedGraph(object):
    ...
    def build_join_tree(self):
        '''Build a join tree from
        an undirected graph.'''
        cliques, elimination_ordering = triangulate(
            self)
        jt = build_join_tree_from_cliques(
            cliques)
        return jt
...
def build_join_tree_from_cliques(cliques):
    '''This differs from the Join Tree
    construction in bbn.py in that
    it returns a 'normal' undirected
    graph. '''
...
def build_undirected_graph(*args):
    '''Build a raw undirected graph.
    We distinguish between a 'graph'
    and a 'graphical model' by the
    fact that a graph is a simple
    structure of edges and vertices.
    A model on the other hand has
    additional metadata, for example
    nodes in a model may have attached
    functions which specify how
    connected nodes interact with
    each other.'''
    nodes = dict()
    if isinstance(args[0], list):
        # Assume the nodes were all
        # passed in as a list in the first
        # argument. This makes it possible
        # to build very large graphs with
        # more than 255 nodes, since
        # Python functions are limited to
        # 255 arguments.
        args = args[0]

    undirected_graph = UndirectedGraph(nodes)
...
class SepSet(object):
    ...
    def insert(self, forest):
        ...
        ss_node = JoinTreeSepSetNode('%s_%s' % (
            self.X, self.Y), self)
...
class JoinTreeSepSetNode(UndirectedNode):
    ...
    def __repr__(self):
        return '<JoinTreeSepSetNode: %s_%s>' % (
            self.sepset.X, self.sepset.Y)
