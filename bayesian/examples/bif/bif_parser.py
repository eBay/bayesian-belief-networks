...
def parse(filename):
    """Parses the .bif file with the given name (exclude the extension from the argument)
...
def create_bbn():
    ...
    return g
"""
          % {'funcs' : ''.join(c for c in str(functions) if c not in "'\""), 'vars' : str(variables), 'name' : filename})
    outfile.close()
    return module_name
